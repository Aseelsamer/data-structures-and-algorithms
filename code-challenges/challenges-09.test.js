...
const createServer = () => {
  ...
  // Routes go here
  // Solution code here...
  server.get('/events', getCurrentEvents);
...
const mapCurrentEvents = () => {
  ...
  return superagent.get(url)
    .then(eventData => {
      let eventObject =  eventData.body.data.map( w => {
        let eventObject = new Event(w);
        return eventObject;
      });
...
const reversedString = (str) => {
  // Solution code here...
  let reversedStr = str.reduce((acc, val, idx) => {
    return val + acc
